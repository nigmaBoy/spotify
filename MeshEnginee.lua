...
function MeshEngine.Init(AudioPlayer, Players, RunService, MeshVisualizerPresets, getMeshEnabled)
    ...
    local function lerpMeshStageColors(alpha)
        local stages = MeshVisualizerPresets.ColorStages or {}
        local numStages = #stages
        if numStages == 0 then return Color3.new(1, 0, 0) end
    ...
    local function rebuildMeshTrigTables()
        ...
        local segments = MeshVisualizerPresets.RadialSegments or 100
        for s = 1, segments do
    ...
    local function RebuildMeshVisualizer()
        ...
        local numLayers = MeshVisualizerPresets.NumColorLayers or 20
    ...
    local function StartMeshVisualizer()
        return RunService.RenderStepped:Connect(function(deltaTime)
            ...
            local numRings = presets.NumRings or 20
            local radialSegments = presets.RadialSegments or 100
            local maxRadius = presets.MaxRadius or 6
            local maxPeakHeight = presets.MaxPeakHeight or 2.5
            local generalReactivity = presets.GeneralReactivity or 1.15
            local centerReactivity = presets.CenterReactivity or 1.2
            local outerReactivity = presets.OuterReactivity or 1.00
            local edgeFalloffPower = presets.EdgeFalloffPower or 3.2
            local numColorLayers = presets.NumColorLayers or 20
            ...
            local centerPos = hrp.Position - Vector3.new(0, presets.CenterOffsetY or 2.95, 0)
            ...
            for r = 1, numRings do
                ...
                for s = 1, radialSegments do
                    local x = (meshCosTable[s] or 0) * currentRadius
                    local z = (meshSinTable[s] or 0) * currentRadius
            ...
            local centerRelativeHeight = gridVertices[1] and gridVertices[1][1] and (gridVertices[1][1].Y - centerPos.Y) or 0
