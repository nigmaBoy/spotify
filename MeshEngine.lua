...
function MeshEngine.Start(AudioPlayer, Players, RunService, MeshVisualizerPresets, MeshVisualizerEnabled)
    ...
    local function RebuildMeshVisualizer()
        ...
        for i = 1, numLayers do
            ...
            wf.Visible = MeshVisualizerEnabled
    ...
    RebuildMeshVisualizer()

    local renderConnection = RunService.RenderStepped:Connect(function(deltaTime)
        if not MeshVisualizerEnabled or not MeshColorLayers then return end
    ...
    return {
        ...
        SetEnabled = function(enabled)
            MeshVisualizerEnabled = enabled
            for _, wf in ipairs(MeshColorLayers) do
                wf.Visible = enabled
                if not enabled then wf:Clear() end
            end
        end,
